...
def _fetch_sub(state: ArchitecturalState, address: int) -> DecodedInstruction:
    state._memory.write(0x000, 0x40 | ((address >> 8) & 0x0F))
    state._memory.write(0x001, address & 0xFF)
    fetched = state.fetch_instruction()
    assert fetched == DecodedInstruction(Opcode.SUB, address)
    return fetched
...
def test_sub_result_boundary_cases(a: int, operand: int, expected: int) -> None:
    state = ArchitecturalState()
    state._a.load(a)
    state._memory.write(0xABC, operand)

    state.execute_instruction(DecodedInstruction(Opcode.SUB, 0xABC))

    assert state.a == expected


@pytest.mark.parametrize("address", [0x000, 0x001, 0x0FF, 0x100, 0xABC, 0xFFE, 0xFFF])
def test_sub_reads_the_exact_full_12_bit_operand_address(address: int) -> None:
    state = ArchitecturalState()
    fetched = _fetch_sub(state, address)
...
def test_sub_reads_current_sram_value_on_repeated_execution() -> None:
    state = ArchitecturalState()
    instruction = DecodedInstruction(Opcode.SUB, 0xABC)
...
def test_sub_defines_all_flags_regardless_of_previous_mask(defined: FlagsDefinedMask) -> None:
    state = ArchitecturalState()
    state._a.load(0x80)
    state._memory.write(0xABC, 0x01)
    state._flags = FlagsSnapshot(FlagValues(False, False, True, False), defined)

    state.execute_instruction(DecodedInstruction(Opcode.SUB, 0xABC))

    assert state.flags.values == FlagValues(False, True, False, True)
    assert state.flags_defined_mask == FlagsDefinedMask.all()


def test_sub_preserves_pc_ir_sram_and_halt_after_fetch() -> None:
    state = ArchitecturalState()
    fetched = _fetch_sub(state, 0xABC)
...
def test_sub_instruction_fetched_across_boundary_preserves_post_fetch_pc() -> None:
    state = ArchitecturalState()
    state._pc.load(0xFFF)
    state._memory.write(0x123, 0xA6)
    state._memory.write(0xFFF, 0x41)
    ...
    assert fetched == DecodedInstruction(Opcode.SUB, 0x123)
    assert state.a == 0x59
    assert state.pc == 0x001


def test_sub_exhaustively_matches_approved_alu_and_flags_policy() -> None:
    state = ArchitecturalState()
    instruction = DecodedInstruction(Opcode.SUB, 0xABC)
    ...
    for a in range(0x100):
        for operand in range(0x100):
            ...
            state._memory.write(0xABC, operand)
            state.execute_instruction(instruction)
            expected = evaluate(ALUMode.SUB, a, operand)
...
def test_sub_does_not_create_a_public_b_register_or_alu_mode_boundary() -> None:
    state = ArchitecturalState()
